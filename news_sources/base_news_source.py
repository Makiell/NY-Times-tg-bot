...
import requests
from news_sources.data_types import News, NewsColorsAndFonts
...
class BaseNewsSource(ABC):
    ...
    def get_one_news(self) -> Optional[News]:
        ...
        for one_news in all_news:
            if storage.exists_element(element=one_news) or self._is_dublicated_news(one_news):
                continue
    ...
    def constract_caption(self, news: News) -> str:
        return f"{news.summary}\n\n{self._get_footer()}"

    def _get_footer(self) -> str:
        return f"#{self.HASHTAG} | {formatting.hlink('Makiell News', 'https://t.me/+BlLAJkS2dp03ZmYy')}"
    ...
    @staticmethod
    def _is_dublicated_news(one_news: News) -> bool:
        storage = NewsStorage()
        ...
        cosine_sin_values = cosine_similarity(tfidf_matrix, tfidf_matrix)[-1][:-1]

        threshold = float(0.3)
        return any([cosine_sin_value for cosine_sin_value in cosine_sin_values if cosine_sin_value > threshold])